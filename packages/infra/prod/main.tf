...
resource "cloudflare_r2_bucket" "sonaura-r2-marketing-bucket-production" {
...
resource "cloudflare_pages_project" "sonaura-marketing-pages-production" {
  account_id        = var.account_id
  name              = "marketing-production"
...
resource "cloudflare_pages_domain" "sonaura-marketing-pages_domain-production" {
  account_id   = var.account_id
  domain       = "dev.${var.domain}"
  project_name = cloudflare_pages_project.sonaura-marketing-pages-production.name
}